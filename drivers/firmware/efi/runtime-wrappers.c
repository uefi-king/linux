	...
	case EFI_QUERY_CAPSULE_CAPS:
		status = efi_call_virt(query_capsule_caps,
				       ...
				       *(unsigned long *)arg2, (u64 *)arg3,
				       (int *)arg4);
		break;
	case EFI_GET_UPTIME_SECONDS:
		status = efi_call_virt(get_uptime_seconds, (unsigned long *)arg1);
		break;
...
static efi_status_t virt_efi_get_uptime_seconds(unsigned long *seconds)
{
	efi_status_t status;
	...
	if (down_interruptible(&efi_runtime_lock))
		return EFI_ABORTED;
	status = efi_queue_work(EFI_GET_UPTIME_SECONDS, seconds,
				NULL, NULL, NULL, NULL);
	up(&efi_runtime_lock);
	return status;
}
...
void efi_native_runtime_setup(void)
{
	...
	efi.get_uptime_seconds = virt_efi_get_uptime_seconds;
